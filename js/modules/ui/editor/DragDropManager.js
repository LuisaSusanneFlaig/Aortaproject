/**
 * DragDropManager handles drag and drop interactions in the editor.
...
export class DragDropManager {
    ...
    setupListeners() {
        // Global listeners for start and end
        ...
        // Story interactions (The "Infocard" targets)
    ...
    handleDragStart(event) {
        ...
        if (paletteItem) {
            ...
            paletteItem.classList.add('is-dragging-source');
            return;
        }

        // Dragging existing element in Panel
        ...
        // Dragging existing section in Panel
    ...
    handleDragEnd() {
        // Cleanup all dragging states
        document.querySelectorAll('.is-dragging, .is-drop-target, .is-dragging-source, .is-dragging-story, .editor-drop-target')
            .forEach(node => node.classList.remove('is-dragging', 'is-drop-target', 'is-dragging-source', 'is-dragging-story', 'editor-drop-target'));
    ...
    writeDragData(event, data) {
        ...
        if (data.kind === 'palette') {
            ...
            ghost.textContent = data.type.toUpperCase();
            ghost.style.cssText = 'padding: 8px 12px; background: #ff4444; color: white; border-radius: 4px; font-weight: bold; font-size: 12px; position: absolute; top: -1000px;';
    ...
    handleStoryDragOver(event) {
        const data = this.readDragData(event);
        if (!data) return;
        if (data.kind !== 'palette' && data.kind !== 'element') return;
        ...
        const step = event.target.closest('.step');
        if (step) {
            // Visual feedback on the "Infocard"
    ...
    /**
     * Setups an element in the story to be draggable back to the editor or within the story.
     */
